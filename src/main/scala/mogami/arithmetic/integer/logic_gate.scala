...
class LogicGate extends Module {
  ...
  val res_normal_s = Vec(2, Bool) // The classification, assumed that it's not NaN
  ...
  val res_normal_d = Vec(2, Bool) // The classification, assumed that it's not NaN
  ...
  output_vec(7, 4) := Mux(io.input.flags(3), fp_output(0), fp_output(1))
  io.output.output1 := reduce_mux(io.input.flags(2, 0), output_vec)
